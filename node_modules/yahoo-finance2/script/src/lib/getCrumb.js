...
async function _getCrumb(cookieJar, fetch, fetchOptionsBase, logger, url = "https://finance.yahoo.com/quote/AAPL", develOverride = {
    ...
    if (depth > MAX_CONSENT_REDIRECT_DEPTH) {
        throw new Error("Too many consent redirects while fetching Yahoo crumb (max " +
    ...
    await processSetCookieHeader((0, headers_js_1.getSetCookieHeaders)(response.headers), url);
    ...
    if (location) {
        if (location.match(/guce.yahoo/)) {
            ...
            if (consentLocation) {
                ...
                if (!(await processSetCookieHeader((0, headers_js_1.getSetCookieHeaders)(collectConsentSubmitResponse.headers), consentLocation))) {
                ...
                if (!(await processSetCookieHeader((0, headers_js_1.getSetCookieHeaders)(copyConsentResponse.headers), collectConsentSubmitResponseLocation))) {
...
function getCrumb(cookieJar, fetch, fetchOptionsBase, logger, notices, url = "https://finance.yahoo.com/quote/AAPL", __getCrumb = _getCrumb) {
    ...
    const state = crumbState(cookieJar);
    if (!state.promise) {
        state.promise = Promise.resolve(__getCrumb(cookieJar, fetch, fetchOptionsBase, logger, url)).catch((error) => {
            state.promise = null;
            throw error;
        });
    }
